...
const app = new Hono()
    ...
    .get(
        ...
        async (c) => {
            ...
            const assignmeIds = tasks.documents.map((task) => task.assignmeId);
            ...
            const members = await databases.listDocuments(
                DATABASE_ID,
                MEMBERS_ID,
                assignmeIds.length > 0 ? [Query.contains("$id", assignmeIds)] : [],
            );

            const assignmes = await Promise.all(
            ...
            const populatedTasks = tasks.documents.map((task) => {
                ...
                const assignme = assignmes.find(
                    (assignme) => assignme.$id === task.assignmeId,
                );
